...
class StanleyAvoidance(Node):
    ...
    def odom_callback(self, pose_msg: Odometry):
        ...
        #             f'Could not transform {to_frame_rel} to {from_frame_re    l}: {ex}')
        #         return
    ...
    def populate_occupancy_grid(self, ranges, angle_increment, angle_min=None):  

        """  
        Populate occupancy grid using lidar scans and save  
        the data in class member variable self.occupancy_grid.  
        """ 
        self.occupancy_grid = np.full(shape=(self.grid_height, self.grid_width),   
        fill_value=self.IS_FREE, dtype=int)  
        ...
        if angle_min is not None:  
            angles = indices * angle_increment + angle_min  
            mask = (angles < np.radians(90)) | (angles > np.radians(45))  
            ranges = ranges[mask]  
            angles = angles[mask]  
      
            thetas = angles - self.ANGLE_OFFSET  
        else:
        ...
        occupied_indices = np.where((i >= 0) & (i < self.grid_height) &   
    ...
    def scan_callback(self, scan_msg):  
        ...
        # populate occupancy grid with filtering  
        self.populate_occupancy_grid(scan_msg.ranges, scan_msg.angle_increment, scan_msg.angle_min)  
