...
def generate_launch_description():
  ...
  mpc_node = Node(
  	package='mpc',
        ...
        output='screen',
        remappings=[  
        ('/drive', '/mpc/drive'),  # 이 줄 추가  
        ],  
        parameters=[
          
               {"real_environment": True},          
               {"csv_file": "/home/meric/f1tenth_ws_rl/src/waypoint_generator/src/map_1115"},      # CSV file name
               ...
               {"wheelbase": 0.25},                   # Your vehicle wheelbase length (cm)
               {"min_steer": -0.4363},                # Your vehicle minimum steering angle (rad)
               {"max_steer": 0.4363},                 # Your vehicle maximum steering angle (rad)
               {"max_steer_rate": 3.142},             # Your vehicle maximum steering rate (rad/s)
               {"max_speed": 2.0},                    # Your vehicle maximum speed (m/s)
